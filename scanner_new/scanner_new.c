...
void skipComment() {
  while (1) {
    ...
    if (currentChar == -1) { // End of File
      ...
      if (currentChar == -1) { // End of File
        error(ERR_ENDOFCOMMENT, lineNo, colNo); // Comment end with EOF
      } else if (charCodes[currentChar] == CHAR_SLASH) { // Next is right parenthesis
        // End of comment
        readChar();
        return;
      }
...
Token* readIdentKeyword(void) {
  ...
  while (charCodes[currentChar] == CHAR_LETTER || charCodes[currentChar] == CHAR_DIGIT || charCodes[currentChar] == CHAR_UNDER) {
...
Token* readConstChar(void) {
  ...
  if (currentChar == -1) { // End of File
	...
	case CHAR_BSLASH:
		...
		if (charCodes[currentChar] == CHAR_SINGLEQUOTE || charCodes[currentChar] == CHAR_BSLASH) {
...
//       readChar();
  ...
  case CHAR_RSQUARE:
    token = makeToken(SB_RSQUARE, lineNo, colNo);
    readChar();
    return token;
  case CHAR_LSQUARE:
    token = makeToken(SB_LSQUARE, lineNo, colNo);
    readChar();
    return token;
  case CHAR_GT:
    // Token Greater
...
void printToken(Token *token) {
  ...
  case SB_LSQUARE: printf("SB_LSQUARE\n"); break;
  case SB_RSQUARE: printf("SB_RSQUARE\n"); break;
  case SB_UNDER: printf("SB_UNDER\n");break;
